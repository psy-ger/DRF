...
- Додано поле `user` до моделі `Book` — зберігається автор (той, хто створив запис). Поле зчитується в серіалізаторі як `user` (username).
- При створенні книги через API `user` автоматично ставиться в поточного автентифікованого користувача.
- Додано сортування (ordering) за полями `publication_year` та `title` через параметр `?ordering=` (наприклад `?ordering=publication_year` або `?ordering=-title`).
- Додано кінцеві точки для реєстрації та входу:
  - `POST /api/auth/register/` — реєстрація користувача, повертає токен.
  - `POST /api/auth/login/` — вхід, повертає токен (DRF Token).
- Оновлено тести — додано перевірки для реєстрації, логіну, встановлення `user` при створенні книги та сортування.
